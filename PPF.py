...
# overload detection
def get_overload_rows(result_file,  threshold=70, overload_file="overload_lines/overload_line.xlsx"):
...
def get_overload_rows2(result_file,  threshold=70, overload_file="overload_lines/overload_line2.xlsx"):
#load result and compare with treshold
    df_res_line = pd.read_excel(result_file, sheet_name="res_line")
    overload_rows = df_res_line[df_res_line["loading_percent"] > threshold]
...
def plot_loading_percent_heatmap(file1, file2, label1, label2, sheet_name="res_line", threshold=70):
    ...
    heatmap_data = pd.DataFrame([df1["loading_percent"].values, df2["loading_percent"].values], 
                                index= [label1, label2], columns= [f" Line {i}" for i in df1.index])
    ...
    ax1 = sns.heatmap(heatmap_data, annot=True, cmap="coolwarm", fmt=".1f",
                    cbar_kws={"label": "Loading Percent (%)"}, linewidths=0.5, linecolor="gray"
                    )
    ...
    for y in range(heatmap_data.shape[0]):
        for x in range(heatmap_data.shape[1]):
            if overload_mask.iloc[y, x]:
                ax1.add_patch(plt.Rectangle((x, y), 1, 1, fill=False, edgecolor="black", lw=2))
...
def plot_Overload_lines_heatmap(file1, file2, overload_rows, label1, label2, sheet_name = "Sheet", threshold= 70):
    if overload_rows.empty:
    ...
    # Reshape into one row with each column representing a line index
    heatmap_data_2= pd.DataFrame([df_overload1["loading_percent"].values, df_overload2["loading_percent"].values],
                                 index= [label1, label2],
    ...
    # plot heatmap 
    plt.figure(figsize=(12, 4))
    ax = sns.heatmap(heatmap_data_2, annot=True, cmap="coolwarm", fmt=".1f",
                    cbar_kws={"label": "Loading Percent (%)"}, linewidths=0.5, linecolor="gray"
                    )
...
if __name__ == "__main__":
    ...
    # Full Heatmap 
    plot_loading_percent_heatmap("results/Result.xlsx", "results/Result2.xlsx",
                                 label1="Scenario A - Before Battery Installation",
                                 label2="Scenario B - After Battery Installation",
                                 threshold=70)
    # Overloaded Lines Only Heatmap
    plot_Overload_lines_heatmap("results/Result.xlsx", "results/Result2.xlsx",
                                overload_rows,
