...
if len(line_stats) == len(griddf_lines):
    ...
    griddf_lines = griddf_lines.merge(line_stats, left_on="index", right_index=True, how="left")
    
    
# Extract coordinates (since Bokeh can't handle shapely geometries)
griddf_lines["x0"] = griddf_lines.geometry.apply(lambda g: g.coords[0][0] if g and not g.is_empty else np.nan)
griddf_lines["y0"] = griddf_lines.geometry.apply(lambda g: g.coords[0][1] if g and not g.is_empty else np.nan)
griddf_lines["x1"] = griddf_lines.geometry.apply(lambda g: g.coords[-1][0] if g and not g.is_empty else np.nan)
griddf_lines["y1"] = griddf_lines.geometry.apply(lambda g: g.coords[-1][1] if g and not g.is_empty else np.nan)

# Convert to Web Mercator for plotting
griddf_lines["x0"], griddf_lines["y0"] = web_mercator(griddf_lines["x0"], griddf_lines["y0"])
griddf_lines["x1"], griddf_lines["y1"] = web_mercator(griddf_lines["x1"], griddf_lines["y1"])
...
subs_pt_df["x"], subs_pt_df["y"] = web_mercator(subs_pt_df["Longitude"], subs_pt_df["Latitude"])
...
p = figure(
    ...
    title="Interactive Grid Mean Line Loading and Substation Map",
    width=1200,
    height=900,
...
if "mean_loading%" in griddf_lines.columns:
    color_mapper = LinearColorMapper(palette=YlOrRd9[::-1], 
                                     low=griddf_lines["mean_loading%"].min(),
                                     high=griddf_lines["mean_loading%"].max()
                                     )
    
    p.segment(
        "x0", "y0", "x1", "y1",
        source = grid_lines,
        color = {"field": "mean_loading%", "transform": color_mapper},
        line_width=3,
        legend_label="Line Mean Loading (%)",
    )
    color_bar =ColorBar(
    ...
    p.segment("x0", "y0", "x1", "y1", source=grid_lines, color="gray", line_width=2, legend_label="Lines")
